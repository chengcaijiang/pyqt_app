...
class QueryDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.setFixedSize(800, 600)  # 增加窗口高度以适应新的按钮
    ...
    def display_result(self, data):
        """显示查询结果"""
        # 清除旧的内容
        self.clear_layout(self.result_group.layout())

        # 创建新的内容布局
        layout = QVBoxLayout()  # 使用垂直布局以容纳更多信息

        # 照片显示
        photo_label = self.create_photo_label(data[6])
        layout.addWidget(photo_label)

        # 用户信息显示
        info_layout = self.create_info_layout(data)
    ...
    def create_photo_label(self, photo_path):
        """创建照片标签"""
        photo_label = QLabel()
        if os.path.exists(photo_path):
            pixmap = QPixmap(photo_path).scaled(200, 200, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def create_info_layout(self, data):
        """创建用户信息布局"""
